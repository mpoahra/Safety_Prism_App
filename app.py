...
app.config["SQLALCHEMY_DATABASE_URI"] = os.getenv("DATABASE_URL", f"sqlite:///{BASE_DIR / 'safety_prism.db'}").replace("postgres://", "postgresql://", 1)
    ...
    for row in reader:
        try:
            if kind == "reports":
                ...
                if report is None:
                    report = Report(external_id=external_id, description=description); db.session.add(report); inserted += 1
                else: updated += 1
                ...
                if not report: errors += 1; continue
                title = _pick(row, "title", "action", "اقدام اصلاحی", "شرح اقدام") or "اقدام اصلاحی"
                action = CorrectiveAction(report_id=report.id, title=title); db.session.add(action); inserted += 1
                ...
                if due:
                    try: action.due_date = date.fromisoformat(due[:10])
                    except ValueError: pass
            elif kind == "people": inserted += 1
        except Exception: errors += 1
    db.session.commit(); return {"inserted": inserted, "updated": updated, "errors": errors}

@app.get("/")
def index(): return render_template("dashboard.html")

@app.get("/legacy")
def legacy(): return send_from_directory(BASE_DIR, "index.html")

@app.get("/api/health")
def health(): return jsonify({"status":"ok","service":"Safety Prism HSE","time":datetime.utcnow().isoformat()})

@app.get("/api/dashboard/analytics")
def dashboard_analytics():
    reports, actions = Report.query.all(), CorrectiveAction.query.all()
    months = {}
    for r in reports: months[r.created_at.strftime("%Y-%m")] = months.get(r.created_at.strftime("%Y-%m"),0)+1
    labels = sorted(months)[-12:]
    status = {"OPEN":0,"CLOSED":0,"OVERDUE":0}
    for a in actions:
        if a.is_overdue: status["OVERDUE"] += 1
        elif a.status == "CLOSED": status["CLOSED"] += 1
        else: status["OPEN"] += 1
    causes = {}
    for r in reports:
        if r.root_cause: causes[r.root_cause] = causes.get(r.root_cause,0)+1
    pareto = sorted(causes.items(), key=lambda x:x[1], reverse=True)[:10]
    types = {"UNSAFE_ACT":0,"UNSAFE_CONDITION":0}
    for r in reports:
        k=(r.observation_type or "").upper()
        if "ACT" in k or "عمل" in k: types["UNSAFE_ACT"] += 1
        elif "CONDITION" in k or "شرایط" in k: types["UNSAFE_CONDITION"] += 1
    return jsonify({"kpi":{"reports":len(reports),"open_reports":sum(r.status!="CLOSED" for r in reports),"actions":len(actions),"overdue":status["OVERDUE"],"closed_actions":status["CLOSED"]},"trend":{"labels":labels,"reports":[months[m] for m in labels],"actions":[0 for _ in labels]},"action_status":{"labels":["باز","بسته‌شده","معوق"],"values":[status["OPEN"],status["CLOSED"],status["OVERDUE"]]},"pareto":{"labels":[x[0] for x in pareto],"values":[x[1] for x in pareto]},"act_condition":{"labels":["عمل ناایمن","شرایط ناایمن"],"values":[types["UNSAFE_ACT"],types["UNSAFE_CONDITION"]]}})

@app.post("/api/import/<kind>")
def api_import(kind):
    if kind not in {"reports","actions","people"}: return jsonify({"error":"نوع فایل نامعتبر است"}),400
    f=request.files.get("file")
    if not f or not f.filename.lower().endswith(".csv"): return jsonify({"error":"یک فایل CSV ارسال کنید"}),400
    return jsonify({"ok":True,"kind":kind,**import_csv(f,kind)})

@app.post("/api/actions/<int:action_id>/verify")
def verify_action(action_id):
    action=db.session.get(CorrectiveAction,action_id)
    if not action:return jsonify({"error":"اقدام پیدا نشد"}),404
    action.status="CLOSED"; action.verified_at=datetime.utcnow(); action.escalation_level=0; db.session.commit()
...
def reports():
    ...
    return jsonify([{"id":r.id,"external_id":r.external_id,"reporter":r.reporter,"department":r.department,"observation_type":r.observation_type,"description":r.description,"risk_level":r.risk_level,"status":r.status,"created_at":r.created_at.isoformat()} for r in rows])
